...
c.title="$msg.get('platform.core.rendering.error.readTechnicalInformation')";
...
var d=new RegExp("H[1-"+$xwiki.getSectionEditingDepth()+"]");
for(var c=0;
c<b.length;
c++){var g=$(b[c]);
...
}},onFailure:function(){new a.widgets.Notification("$msg.get('core.create.ajax.error')","error",{inactive:true}).show()
}});
d.stop()
})
...
document.observe("xwiki:dom:loaded",function(){var d={documents:{script:XWiki.Document.getRestSearchURL("scope=name&number=10&media=json&"),varname:"q",icon:"$xwiki.getSkinFile('icons/silk/page_white_text.png')",noresults:"Document not found",json:true,resultsParameter:"searchResults",resultId:"id",resultValue:"pageFullName",resultInfo:"pageFullName"},spaces:{script:XWiki.Document.getRestSearchURL("scope=spaces&number=10&media=json&"),varname:"q",icon:"$xwiki.getSkinFile('icons/silk/folder.png')",noresults:"Space not found",json:true,resultsParameter:"searchResults",resultId:"id",resultValue:"space",resultInfo:"space"},users:{script:XWiki.currentDocument.getURL("get","xpage=uorgsuggest&classname=XWiki.XWikiUsers&wiki=local&uorg=user&"),varname:"input",icon:"$xwiki.getSkinFile('icons/silk/user.png')",noresults:"User not found"},groups:{script:XWiki.currentDocument.getURL("get","xpage=uorgsuggest&classname=XWiki.XWikiGroups&wiki=local&uorg=group&"),varname:"input",icon:"$xwiki.getSkinFile('icons/silk/group.png')",noresults:"Group not found"}};
